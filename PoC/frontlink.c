...
#endif
{
  ...
  if (is_small_request(nb))  /* Faster version for small requests */
  {
         ...
         ((victim = last(bin_at(idx+1))) != bin_at(idx+1)))
    {
      ...
      fprintf(stderr, "malloc(%lu) returning %p\n", bytes, chunk2mem(victim)); fflush(stderr);
...
int main(int argc, char **argv)
{
  ...
  third = malloc(12);
  fprintf(stderr, "Allocating space for fourth\n"); fflush(stderr);
  fourth = malloc(666);
  fprintf(stderr, "Allocating space for fifth\n"); fflush(stderr);
  fifth = malloc(1508);
